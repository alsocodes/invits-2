...
const Cover2In = ({ guestData }) => {
  return (
    <div id='the-home' className={`w-full h-full overflow-hidden bg-[#dbf4e1]`}>
        ...
        <div className='h-1/3 bg-[#dbf4e1] relative p-4 select-none'>
          <div className='absolute -top-2 md:top-0 w-full left-0 right-0 px-4'>
            ...
            <div className='text-center w-[300px] text-[#226b4b] px-4 py-2 mx-auto mt-4 border border-primary rounded-md'>
              ...
              <div className='font-semibold h-6'>{guestData?.name || ''}</div>
              <div className='text-sm italic h-6'>
                {guestData?.address ? `Di ${guestData?.address}` : ''}
              </div>
            </div>
          </div>
        </div>
