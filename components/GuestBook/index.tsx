...
const noBride = '6282331348786';
const noGroom = '6288235503445';
const GuestBook = ({ guestData, messages, getAllMessages }) => {
  // console.log('xna messages', messages);
  const {
    register,
    handleSubmit,
    formState: { errors },
  ...
  const [birdeOrGroom, setBrideOrGroom] = useState('');
  const [loading, setLoading] = useState(false);

  useEffect(() => {
    setValue('message', guestData?.message?.content);
    setValue('name', guestData?.name);
  }, [guestData]);

  const onKirimKonfirmasi = (data) => {
    setLoading(true);
    const dataOlah = {
      ...data,
      confirm: data?.confirm === '1',
    };
    apiCall('POST', `/api/guest/${guestData?.guestId}`, dataOlah).then(
      (data) => {
        getAllMessages();
        setLoading(false);

        let textConfrim = "_Assalamu'alaikum_, \r\nSaya " + guestData?.name;
        textConfrim +=
          ' ingin mengkonfirmasikan kehadiran kami pada acara pernikahan Vega & Firdaus, bahwa';
        if (dataOlah?.confirm) textConfrim += ' InsyaAllah Saya akan Datang';
        else textConfrim += ' Maaf, Saya tidak bisa datang.\r\n';
        textConfrim +=
          'Saya Ucapkan : _*' + dataOlah?.message + '* _.\r\nTerimakasih';
        const no = birdeOrGroom === 'bride' ? noBride : noGroom;
        window.location.href = `https://wa.me/${no}?text=${encodeURIComponent(
          textConfrim
        )}`;
      }
    );
  ...
  return (
    <div
      id='the-book'
      className='px-4 py-16 md:py-24 relative w-full min-h-screen overflow-hidden'
    >
      ...
      >
        <div className='card-body'>
          <form onSubmit={handleSubmit(onKirimKonfirmasi)}>
            <input
              readOnly
              type='text'
            ...
            <textarea
              ...
              {...register('message', { required: 'harus diisi' })}
            />
            <div className='form-control'>
              <label className='label cursor-pointer justify-start gap-2'>
                <input
                  ...
                  value={1}
                  defaultChecked={guestData?.message?.confirm}
                  {...register('confirm', { required: 'harus dipilih' })}
                />
            ...
            <div className='mt-4 md:flex gap-4'>
              ...
              <button
                ...
                onClick={() => setBrideOrGroom('groom')}
              >
                Kirim WA Mempelai Pria
              </button>
            </div>
          </form>
      ...
      <div
        data-aos='fade-up'
        data-aos-duration='500'
        className='text-center text-3xl font-semibold text-[#226b4b] mb-2 mt-12 xl:mt-20'
        style={{ fontFamily: 'Dancing Script' }}
      >
        Ucapan & Doa
      </div>
      <div
        data-aos='fade-up'
        data-aos-duration='500'
        className='card w-full max-h-[500px] min-h-[150px] 
        overflow-y-auto md:w-1/2 md:max-w-[700px] mx-auto bg-white shadow-xl mt-6'
      >
        <div className='card-body p-4'>
          <ul>
            {messages?.map((a) => {
              const splits = a?.guestName?.split(' ');

              const initial =
                splits?.length === 1
                  ? a?.guestName?.substr(0, 2)
                  : `${splits[0]?.substr(0, 1)}${splits[1]?.substr(0, 1)}`;
              ...
              return (
                <li key={a?._id}>
                  <div className='flex gap-4 border-b border-primary p-2'>
                    ...
                    <div>
                      <div className='text-gray-600'>{a?.guestName}</div>
                      <div className='mt-1 text-md italic'>{a?.content}</div>
                    </div>
                  </div>
                </li>
              );
